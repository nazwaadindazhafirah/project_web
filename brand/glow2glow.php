...
$produk = mysqli_query($conn, "SELECT * FROM produk WHERE brand = 'Glow2Glow'");
?>
<!DOCTYPE html>
<html>
<head>
    <title>Glow2Glow - Nazwaazhf Skincare</title>
...
<div class="container mt-4">
    ...
    <h2>Produk Glow2Glow</h2>
    <div class="row">
        <?php while ($row = mysqli_fetch_assoc($produk)): ?>
        <div class="col-md-4 mb-4">
            <div class="card">
                <img src="../assets/gambar/glow2glow/<?= $row['gambar'] ?>" class="produk-img card-img-top" alt="<?= $row['nama_produk'] ?>">
                <div class="card-body">
                    <h5 class="card-title"><?= $row['nama_produk'] ?></h5>
                    <p class="card-text"><?= $row['deskripsi'] ?></p>
                    <form action="../tambah_keranjang.php" method="post">
                        <input type="hidden" name="id_produk" value="<?= $row['id_produk'] ?>">
                        <button type="submit" name="tambah" class="btn btn-pink btn-pulse btn-bounce w-100">🛒 Tambah ke Keranjang</button>
                    </form>
                </div>
            </div>
        </div>
